...
- **更多状态码** — 补全了缺失的标准码（505 等）、Nginx 4xx 扩展（444、494–499）、Cloudflare 5xx（520–527、530）以及完整的 Cloudflare 1xxx 错误系列（1000–1201）
- **多语言支持（i18n）** — UI 文案和状态码描述均支持多语言，缺少翻译时自动回退到英文。新增语言只需添加 `locales/<locale>/common.json` 和 `content/<locale>/` 文件夹
- **分类 Tab** — 状态码按类别分组（1xx / 2xx / 3xx / 4xx / 5xx / Cloudflare 1xxx），支持 Tab 切换和分类简介
- **图片回退** — 没有专属猫咪图片的状态码自动显示占位图
...
3. 为每种支持的语言添加 `content/<locale>/<code>.md`
...
4. 创建 `app/<locale>/page.tsx` 和 `app/<locale>/status/[status]/page.tsx`，参考现有语言的页面文件修改
