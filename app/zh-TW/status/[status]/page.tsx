...
export default async function Info(props: { params: Promise<{ status: string }> }) {
  ...
  const localizedName = localizedStatusName(statusObj, 'zh-TW');
  ...
  return (
    <>
      ...
      <main>
        <nav>
          <Link href="/zh-TW" className="text-white">{`< ${t.BACK_TO_HOME}`}</Link>
        </nav>
...
export async function generateMetadata(
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/zh-TW/status/${statusObj.code}`,
      languages: buildHrefLangMap(`/status/${statusObj.code}`),
    },
    openGraph: {
      title,
      description,
      url: `/zh-TW/status/${statusObj.code}`,
